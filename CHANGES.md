...
`paper_plots_c_v2_single_case.ipynb`, which had been edited in place:
`annotation_fontsize` 20 -> 40 and `legend_fontsize` 22 -> 44.  Those were the
only substantive differences; the rest of that cell differs from the module
only by the stripped header docstring and the dropped `__main__` guard, both
artefacts of pasting.  The larger key matters more now that Figure 10 is set
in a single column rather than across both.

---

## 2026-09-10 — `Δ b_sys` plot: one statistic per row, not two

Review comment on Figure 10 of the paper: "I don't think you need both types of
point. One or the other is fine (errorbar vs n-sigma intervals)."  Each row was
drawing the graded credible intervals with a median marker *and* a mean ± σ
error bar below it, which is the same information twice.

- **New `statistic` parameter** on `plot_delta_bsys()`, one of `'intervals'`
  (graded bars plus the posterior median), `'mean'` (posterior mean with a
  ±σ error bar) or `'both'` (the previous behaviour).  The default is
  `'intervals'`: the figure's caption claims a parameter is biased at a stated
  confidence level, and it is the credible intervals that support reading that
  off the figure.
- With a single statistic on a row it now sits on the row centre rather than
  dodging above and below, and the mean marker is drawn slightly larger since
  it no longer has to share the row.  `MEAN_OFFSET` applies to `'both'` alone.
- `_legend_handles()` names only what is drawn, and `_axis_limit()` reaches
  only as far as what is drawn — so `'mean'` gets a visibly tighter x axis than
  `'intervals'`, which has to hold the 3σ bars.
- `--statistic` added to the demo command line.

**`paper_plots_c_v2_single_case.ipynb`** — the Figure 10 cell passes
`statistic='intervals'` explicitly.  The figure itself must be regenerated by
re-running that cell against the run outputs; `bsys_delta_plot.pdf` in the
paper repository is still the two-statistic version until then.

**Tests** — nine new in `tests/test_plot_delta_bsys.py` (89 total): the default,
the markers and bars present under each value, the guard on an unknown value,
the key naming only what is drawn, the row centring against the offset of
`'both'`, and `_axis_limit` shrinking when a statistic is dropped.  Full suite:
237 passed.
